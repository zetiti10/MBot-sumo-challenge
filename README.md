...
Tous les fichiers sont commentés pour faciliter la compréhension du programme. La documentation des fonction est affichée lorsqu'elles sont pointées par la souris.

Le programme est réparti en plusieurs fichiers ayant différentes fonctions. Le fichier principal est `main.cpp`, il contient les deux fonctions fondammentales d'un programme Arduino :
...
| Valeur renvoyée | Correspondence                                                                               |
|-----------------|----------------------------------------------------------------------------------------------|
...
Il permet de reçevoir des instructions de la télécommande du kit.
...
| Valeur dans le programme | Correspondence                       |
|--------------------------|--------------------------------------|
| FORWARD                  | Avancer                              |
| BACKWARD                 | Reculer                              |
...
La vitesse est de va de `0` à `255`. Il est aussi possible de renseigner une valeur négative pour effectuer le déplacement inverse.
...
Le contrôle se fait via trois fonction donc les paramètres sont identiques : il faut préciser une valeur d'intensité pour chaque composante du cercle chromatique : le rouge, le vert et le bleu.
...
Les valeurs vont de 0 (éteint) à 255 (pleine luminosité). Il est donc possible d'avoir 16 581 375 nuances !
...
while (1)
{
    // Génration d'un bruit dont la fréquence augmente lentement.
